...
/*
use xuserver\modules\boutique\customers;
$o = new xuserver\modules\boutique\customers();
...
class xs_Examples{
    function _menubar(){
        ...
        foreach ($menus as $menu){
            ...
            <a class='nav-link dropdown-toggle' href='https://getbootstrap.com/' data-toggle='dropdown' aria-haspopup='true' aria-expanded='false'>$menu</a>
    ...
    function admin_users(){
        ...
        $obj=Build("auth_user");
        if(!$session->admin()){
            $obj->properties()->find("#my_lastname, #my_firstname")->select();
            $obj->read();
            echo notify("Accès refusé","danger clear");
            $return = $obj->ui->table();
        }else{
            $obj->read();
            $obj->properties()->find("id, #email,#my_lastname, #my_firstname, #auth_locked,#terms_conditions")->select();
            $test="<div><a href='auth_user-0' method='lock' class='xs-link'>unlock</a> | <a href='auth_user-0' method=\"lock(1)\" class='xs-link'>lock</a></div>";
            $return = $obj->ui->table().$test.$obj->ui->structure();
        }
    ...
    function help_database(){
        ...
        header("Location: http://".$_SERVER["SERVER_NAME"]."/xs-framework/v5/database.php");
        die();
    }
    function help_home(){
